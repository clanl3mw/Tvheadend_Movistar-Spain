...
URL_LIST="https://github.com/clanl3mw/Tvheadend_Movistar-Spain/raw/master/files/Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" 2>>Tvheadend_Movistar-Spain.log
...
if [ "$INSTALL_LIST" = true ]; then
	...
	wget -qO "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" "$URL_LIST" 2>>Tvheadend_Movistar-Spain.log
	if [ $? -eq 0 ]; then
		printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	else
		printf "%s$red%s$end%s\n" "[" "FAILED" "]"
	...
	tar -Jxf "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" -C $TVHEADEND_CONFIG_DIR channel input Picons 2>>Tvheadend_Movistar-Spain.log #--strip-components=1
...
if [ "$INSTALL_GRABBER" = true ]; then
	if [ ! -f "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" ]; then
		printf "%-$(($COLUMNS-10))s"  " * Descargando lista de canales"
		wget -qO "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" "$URL_LIST" 2>>Tvheadend_Movistar-Spain.log
		if [ $? -eq 0 ]; then
			printf "%s$green%s$end%s\n" "[" "  OK  " "]"
	...
	tar -Jxf "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" -C $TVHEADEND_CONFIG_DIR epggrab 2>>Tvheadend_Movistar-Spain.log
...
if [ -f "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" ]; then
	rm "Configuracion_Tvheadend_$REMOTE_LIST_VERSIONtar.xz" 2>>Tvheadend_Movistar-Spain.log
fi
